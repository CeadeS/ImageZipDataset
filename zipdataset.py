...
class ImageZipDataset(torch.utils.data.Dataset):
    def __init__(self, zip_path, info_path, transform=None, target_transform=None, delimiter='\t',
                split='train', n_crossval_split=-1, n_crossval=None, eval_proportion = .2, random_seed=3, load_fn = pd.read_csv, f_key='filename', l_key='label'):
        ...
        print(f"number classes {max(self.classes)}")
